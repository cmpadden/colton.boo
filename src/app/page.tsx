...
export default function Home() {
  ...
  const orbit=useRef({yaw:-0.28,pitch:-0.16,zoom:1});
  ...
  return <main className={styles.page}>
    <canvas ref={canvas} onPointerDown={down} onPointerMove={move} onPointerUp={()=>{pointer.current=null;}}
      ...
      tabIndex={0} aria-label="Ghost. Drag to rotate, scroll to zoom, or use the arrow keys."
    ...
    <footer className={styles.controls}><span>Drag to turn · Scroll to zoom</span><button onClick={reset}>Reset view</button></footer>
  </main>;
}
